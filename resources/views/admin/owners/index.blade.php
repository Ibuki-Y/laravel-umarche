            ...
            <div class="container md:px-5 mx-auto">
              ...
              <div class="lg:w-2/3 w-full mx-auto overflow-auto">
                <table class="table-auto w-full text-left whitespace-no-wrap">
                  <thead>
                    <tr>
                      <th class="md:px-2 md:py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">
                        名前
                      </th>
                      <th class="md:px-2 md:py-3 title-font tracking-wider font-medium text-gray-900 text-sm bg-gray-100 rounded-tl rounded-bl">
                        メールアドレス
                  ...
                  <tbody>
                    @foreach ($owners as $owner)
                      <tr>
                        <td class="md:px-2 md:py-3">{{ $owner->name }}</td>
                        <td class="md:px-2 md:py-3">{{ $owner->email }}</td>
                        <td class="md:px-2 md:py-3">{{ $owner->created_at->diffForHumans() }}</td>
                        <td class="px-2 py-3">
                          <button type="button" onclick="location.href='{{ route('admin.owners.edit', ['owner' => $owner->id]) }}'"
                            class="text-white bg-indigo-400 border-0 py-2 px-6 focus:outline-none hover:bg-indigo-300 rounded text-center">編集</button>
                        </td>
