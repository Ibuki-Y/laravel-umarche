    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
      <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
        <div class="p-6 bg-white border-b border-gray-200">
          <div class="md:flex md:justify-around">
            ...
            <div class="md:w-1/2 ml-4">
              ...
              <div class="flex justify-around items-center">
                ...
                <form method="post" action="{{ route('user.cart.add') }}" class="flex justify-around items-center">
                  @csrf
                  <div class="ml-16">
                    <span class="mr-3">数量</span>
                    <select name="quantity"
                      class="rounded border appearance-none border-gray-300 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-200 focus:border-indigo-500 text-base pl-3 pr-10 mr-8">
                      @for ($i = 1; $i <= $quantity; $i++)
                        <option value="{{ $i }}">{{ $i }}</option>
                      @endfor
                    </select>
                  </div>
                  <button class="flex whitespace-nowrap ml-auto text-white bg-indigo-500 border-0 py-2 px-4 focus:outline-none hover:bg-indigo-600 rounded">カート</button>
          ...
          <div class="mb-4 text-center">ショップ情報</div>
          ...
          <div class="mb-4 text-center">
            <button type="button" data-micromodal-trigger="modal-1" href="javascript:;" class="text-white bg-gray-500 border-0 py-2 px-6 focus:outline-none hover:bg-gray-600 rounded">詳細</button>
          </div>
        </div>
      </div>
    </div>
